...
for (i in 1:nrow(covid_data)) {
  for (j in 1:nrow(tari)) {
    if (covid_data$confirmed[[i]] >0  & !is.na(covid_data$confirmed[[i]]))
    {
      if (tari$tari[[j]] == covid_data$country_code[[i]]) {
        v_prima_zi <- tari$prima_zi[j]
        if(v_prima_zi == as.Date('1970-01-01')) {
          tari$prima_zi[[j]] <- covid_data$report_date[[i]]
          tari$ultima_zi[[j]]<- covid_data$report_date[[i]]
        } else if ( covid_data$report_date[[i]]>tari$ultima_zi[[j]] ) {
          tari$ultima_zi[[j]]<- covid_data$report_date[[i]]
        } 
      }
    }
...
for (i in 1:nrow(covid_data)) {
  for (j in 1:nrow(tari)) {
    if (covid_data$confirmed[[i]] >0  & !is.na(covid_data$confirmed[[i]]))
    {
      if (tari$tari[[j]] == covid_data$country_code[[i]]) {
        v_prima_zi <- tari$prima_zi[[j]]
        if(v_prima_zi == as.Date('1970-01-01') |v_prima_zi>covid_data$report_date[[i]] ) {

                    tari$prima_zi[[j]] <- covid_data$report_date[[i]]
         if (tari$ultima_zi[[j]] == as.Date('1970-01-01'))
          {
          tari$ultima_zi[[j]]<- covid_data$report_date[[i]]
          tari$nr_cazuri[[j]]<- covid_data$confirmed[[i]]
          }
        } else if (( covid_data$report_date[[i]]>tari$ultima_zi[[j]] & covid_data$confirmed [[i]]>tari$nr_cazuri[[j]] )| (covid_data$report_date[[i]]<tari$ultima_zi[[j]] & covid_data$confirmed [[i]]==tari$nr_cazuri[[j]])) {
          tari$ultima_zi[[j]]<- covid_data$report_date[[i]]
          tari$nr_cazuri[[j]]<- covid_data$confirmed[[i]]
