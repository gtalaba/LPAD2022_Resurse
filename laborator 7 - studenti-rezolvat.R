...
covid_new %>%
  select(country_code, report_date, deaths_new) %>%
  inner_join(country__pop_coord,  by = c("country_code"="country_code3")) %>%
  select(-latitude, - longitude) %>%
  mutate(procent_morti = (deaths_new /population)/1000) %>%
  group_by(country_code) %>%
  top_n(1,procent_morti)
...
covid_new %>%
  ...
  transmute(country_name, report_date, population,procent_confirmati = (confirmed_new /population)/1000) %>%
  group_by(country_name) %>%
  top_n(5,procent_confirmati)
